...
public class StudentDaoImpl implements StudentDao {
	...
	public Student updateById(int id) {
		...
		Session session = factory.openSession();
		Student updateStu = session.get(Student.class, id);
		if (updateStu == null) {
			throw new StudentNotFoundException("Student not found");
		}
		...
		updateStu.setFirstName(firstName);
		updateStu.setLastName(lastName);
		updateStu.setEmail(email);

	//	Set<String> set = updateStu.getImages();

		System.out.println("Do you need to change images(yes/no)");
		String c = scanner.next();
		if (c.equals("yes")) {

			updateStu.setImages(null);
			...
			updateStu.setImages(set1);
		}
		session.getTransaction().begin();
		session.merge(updateStu);
		session.getTransaction().commit();
